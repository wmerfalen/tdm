...
CGET(uint16_t,MARINE_SUPPRESS_TICKS);
...
CGET(float,MARINE_SUPPRESSION_TICKS_PER_TIER);
...
CGET(std::string,LOGIN_SCREEN_USERNAME);
...
CGET(uint8_t,MARINE_PIN_DOWN_COUNT_PER_TIER);
CGET(uint8_t,MARINE_EXPLOSIVE_DRONE_COUNT_PER_TIER);
...
CGET(uint8_t,MARINE_REPLENISH_PULSE);
