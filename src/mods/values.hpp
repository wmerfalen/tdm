...
namespace mods::values {
	...
	enum value_key_t : uint16_t {
vk_ALLOW_MOCK_LEVELS = 1,
...
#define MENTOC_UINT32_TUPLE ( \
(BREACHER_REPLENISH_CALL_COUNT,1), \
(GHOST_DISSIPATE_TICKS_DURATION,20), \
(SNIPER_XRAY_SHOT_MAX_COUNT,10) \
)

#define MENTOC_DEFINE_UINT32_IMPL(r,data,MEMBER_TUPLE) \
	static inline uint32_t BOOST_PP_TUPLE_ELEM(2,0,MEMBER_TUPLE) () { return CONSTGET<uint32_t>(BOOST_PP_CAT(vk_,BOOST_PP_TUPLE_ELEM(2,0,MEMBER_TUPLE)),BOOST_PP_TUPLE_ELEM(2,1,MEMBER_TUPLE)); }

#define MENTOC_INITIALIZE_UINT32(SEQUENCE) \
			BOOST_PP_SEQ_FOR_EACH(MENTOC_DEFINE_UINT32_IMPL,~,BOOST_PP_TUPLE_TO_SEQ(BOOST_PP_TUPLE_SIZE(SEQUENCE),SEQUENCE))

MENTOC_INITIALIZE_UINT32(MENTOC_UINT32_TUPLE);
