...
namespace mods::builder::ammobuild {
	...
	using ammobuild_orm_type = mods::orm::mob_equipment;
	struct ammobuild_interface : public slotted_builder<ammobuild_vnum_t,ammobuild_orm_type> {
		...
		ammobuild_interface() {
			...
			get_signatures()["map-assign"] =
			    "{grn}ammobuild{/grn} {red}map-assign <mob-virtual-number> <meq-profile-vnum>{/red}\r\n"
			    "  |--> assigns the specific mob to have the equipment profile of 'meq_vnum'\r\n";
			get_signatures()["map-delete"] =
			    "{grn}ammobuild{/grn} {red}map-delete <mob-virtual-number>...[mob-virtual-number-N]{/red}\r\n"
			...
			get_signatures()["map-list"] =
			    "{grn}ammobuild{/grn} {red}map-list{/red}\r\n"
		...
		status_response_t dispatch_new_command(const std::vector<std::string>& cmd_args,std::string argument) {
			...
			if(vnum < 0) {
				return {0,"{ammobuild new} vnum must be a positive number"};
			}
	...
	SUPERCMD(do_ammobuild) {
		...
		/** map-assign */
		{
			...
			if(args.has_value()) {
				...
				if(cmd_args.size() < 3) {
					...
					r_error(player,"{ammobuild map-assign} Not enough arguments");
					return;
				}
				/**
				...
				if(mvn <= 0) {
					...
					r_error(player,"{ammobuild map-assign} mob vnum must be a positive number");
					return;
				}
				if(eq_vnum <= 0) {
					m_debug("map-assign eq vnum invalid");
					r_error(player,"{ammobuild map-assign} eq vnum must be a positive number");
					return;
				}
				for(auto& m : mods::orm::mob_equipment_map_list()) {
					if(m->mmap_mob_vnum == mvn) {
						...
						if(m->save() < 0) {
							r_error(player,"{ammobuild map-assign} Unable to save existing.");
							m->mmap_mob_equipment_vnum = backup;
							m_debug("map-assign cant save existing");
							return;
						}
						m_debug("map-assign saved existing");
						r_success(player,"{ammobuild map-assign} Saved existing.");
						ENCODE_R("ok");
						return;
					}
				}
				...
				if(ref->save() < 0) {
					...
					r_error(player,"{ammobuild map-assign} Unable to save.");
					return;
				}
				mods::orm::mob_equipment_map_list().emplace_back(std::move(ref));
				r_success(player,"{ammobuild map-assign} Created new mapping and saved");
				ENCODE_R("ok");
		...
		/** map-delete */
		{
			...
			if(args.has_value()) {
				...
				auto cmd_args = args.value();
				if(cmd_args.size() < 2) {
					r_error(player,"{ammobuild map-delete} Not enough arguments");
					return;
				}
				/**
				...
				for(unsigned i=1; i < cmd_args.size(); ++i) {
					...
					if(mvn < 0) {
						...
						r_error(player,"{ammobuild map-delete} mob vnum must be a positive number");
						continue;
					}
					for(auto& m : mods::orm::mob_equipment_map_list()) {
						if(m->mmap_mob_vnum == mvn) {
				...
				r_success(player,CAT("{ammobuild map-delete} Deleted: ",list));
	...
	void init() {
		ADD_BUILDER_COMMAND("admin:loot:ammo:create",do_ammobuild);
		ADD_BUILDER_COMMAND("admin:loot:ammo:delete",do_ammobuild);
		ammobuilder(nullptr);
	}
};
#undef m_debug
