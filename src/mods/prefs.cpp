...
namespace mods {
	...
	void prefs::set(const std::string& key,const std::string& value,struct char_data* ch) {
		struct char_data * c = ch;
		auto named = namify(key,c);
		mods::globals::db->put(named,value);
	}

	std::string prefs::get(const std::string& key,struct char_data* ch) {
		struct char_data * c = ch;
		std::string value;
		auto k = namify(key,c);
	...
	void prefs::dynamic_set(const std::string& key,const std::string& value,const std::string& section,struct char_data* ch) {
		auto k = dynamic_namify(key,section,ch);
		mods::globals::db->put(k,value);
	}
	std::string prefs::dynamic_get(const std::string& key,const std::string& section,struct char_data* ch) {
		auto k = dynamic_namify(key,section,ch);
		std::string value;
