...
namespace mods {
	struct player_scaffolding {
		//using std::shared_ptr<mods::player> = std::shared_ptr<mods::player>;
		//using points_map_t = std::map<std::string,int>;
		//using std::vector<std::shared_ptr<mods::player>>  = std::vector<std::shared_ptr<mods::player>>;
		...
		constexpr static std::size_t OTHER_PLAYER_INDEX_START = 2;
			 ...
			 */
			int initialize_player(std::shared_ptr<mods::player> player,int to_room);
			/**
			 ...
			 * @return std::vector<std::shared_ptr<mods::player>> of all players. first two elements are main_player and target_player respectively. The rest (if any) are all other players you specified.
			 */
			std::vector<std::shared_ptr<mods::player>>& set_players(std::string_view main_player,
			 ...
			 * @returns Returns -1 if the creation failed. get_last_error() can be called on player_scaffolding to find out what happened. Returns 0 on success. Both std::shared_ptr<mods::player> objects are stored in the m_main_player and m_target_player protected member variables on this player_scaffolding object.
			 */
			 ...
			 */
			int set_players(std::shared_ptr<mods::player> main_player,std::shared_ptr<mods::player> target_player);
			/**
			 ...
			 * @return std::shared_ptr<mods::player>
			 */
			std::shared_ptr<mods::player> create_player();
			/**
			 * Resets the passed in player's points (char_data->points.*)
			 *
			 * @param player The std::shared_ptr<mods::player> that you want to modify
			 ...
			 * @return void
			 */
			void reset_points(std::shared_ptr<mods::player> player, std::map<std::string_view,int>* points_map);
			/**
			 * Creates a typical target player that has his descriptor_data muted
			 * @return the player's pointer
			 */
			std::shared_ptr<mods::player> create_muted_player_target();
			
		protected:
			std::vector<std::shared_ptr<mods::player>> m_players;
