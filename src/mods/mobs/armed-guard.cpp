...
#ifdef  __MENTOC_MODS_MOBS_armed_guard_SHOW_DEBUG_OUTPUT__
#define m_debug(a) mentoc_prefix_debug("m|m|cma") << a << "\n";
#define cmem(a) mentoc_prefix_debug("[armed_guard][memory_footprint]") << a << "\n";
#else
#define m_debug(a)
#define cmem(a)
#endif
namespace mods::mobs {
	namespace armed_guard_btree {
		/**
	...
	};// end namespace armed_guard_btree
	void armed_guard::create(const uuid_t& mob_uuid, std::string_view targets) {
		m_debug("armed_guard create on uuid:" << mob_uuid);
		auto p = ptr_by_uuid(mob_uuid);
		if(!p) {
			log("SYSERR: did not find player to populate armed_guard with: %d",mob_uuid);
			return;
		}
		auto g = std::make_shared<armed_guard>(mob_uuid,targets.data());
		g->btree_roam();
		mods::mobs::armed_guard_list().push_front(g);
	}
	void armed_guard::perform_random_act() {
	 ...
	 * @param player
	 */
	void armed_guard::enemy_spotted(room_rnum room,uuid_t player) {
		m_debug("##################################################################################" <<
		        "[armed_guard] enemy spotted:" << room << "\n" <<
		        "##################################################################################");
		//this->spray(player_ptr->get_watching());
		this->last_seen[player] = CURRENT_TICK();
	}
	void armed_guard::set_variation(const std::string& v) {
		for(const auto& type : EXPLODE(v,' ')) {
			std::cerr << green_str("armed_guard::variation:") << type << "\n";
		}
	}
	str_map_t armed_guard::report() {
		return usages();
	}
	str_map_t armed_guard::usages() {
		str_map_t m;
		m = base_usages();
	...
	void armed_guard::set_behavior_tree_directly(const armed_guard::btree_t& t) {
		m_debug("setting tree id directly to: " << t);
		cd()->mob_specials.behaviour_tree = (uint16_t)t;
	}
	bool armed_guard::has_tree() {
		return cd()->mob_specials.behaviour_tree != armed_guard::btree_t::CMA_NONE;
	}
	armed_guard::btree_t armed_guard::get_tree() {
		return (btree_t)cd()->mob_specials.behaviour_tree;
	}
	void armed_guard::btree_none() {
		set_behaviour_tree_directly(armed_guard::btree_t::CMA_NONE);
	}
	void armed_guard::btree_roam() {
		set_behavior_tree_directly(armed_guard::btree_t::CMA_ROAM);

	}
	void armed_guard::btree_hostile() {
		set_behavior_tree_directly(armed_guard::btree_t::CMA_HOSTILE);

	}
	/**
	 * @brief damage_events registered here
	 */
	void armed_guard::setup_damage_callbacks() {
		...
		player_ptr->register_damage_event_callback({de::YOURE_IN_PEACEFUL_ROOM},[&](const feedback_t& feedback,const uuid_t& player) {
			...
			int decision = armed_guard_btree::weighted_direction_decider(player_ptr,nullptr);
			if(decision == -1) {
	...
	bool armed_guard::is_rival(player_ptr_t& player) {
		return false;
	}
	void armed_guard::door_entry_event(player_ptr_t& player) {
		if(player->is_npc()) {
			if(is_rival(player)) {
				btree_roam();
	 ...
	 * @param variation
	 */
	armed_guard::armed_guard(const uuid_t& mob_uuid, std::string_view variation) {
		this->init();
		this->uuid = mob_uuid;
		auto p = ptr_by_uuid(mob_uuid);
		if(!p) {
	...
	void armed_guard::attacked(const feedback_t& feedback) {
	...
	void armed_guard::melee_attack_within_range() {
	...
	void armed_guard::attack(player_ptr_t& player) {
		m_last_attacker = player;
		melee_attack_within_range();
	}
	void armed_guard::move_closer_to_target() {
		uint8_t loops = 1;
	...
	int8_t armed_guard::determine_heading_from_found_victims() {
	...
	bool armed_guard::has_found_item() {
		return m_found_item;
	}
	void armed_guard::set_found_item(bool status) {
		m_found_item = status;
	}
	void armed_guard::found_witness(const mods::scan::vec_player_data_element& data) {
	...
	void armed_guard::clear_scanned_items() {
		m_scanned_items.clear();
	}
	void armed_guard::remember_item(const mods::scan::vec_player_data_element& data) {
	...
	const armed_guard::uuidlist_t& armed_guard::get_remembered_items() const {
		return m_remembered_items;
	}
	void armed_guard::found_item(mods::scan::vec_player_data_element const& item) {
		clear_list_if_count(&m_scanned_items,10);
		m_scanned_items.emplace_back(item);
		cmem("m_scanned_items:" << m_scanned_items.size());
	}
	std::forward_list<std::shared_ptr<armed_guard>>& armed_guard_list() {
		static std::forward_list<std::shared_ptr<armed_guard>> s;
		return s;
	}
	uint8_t armed_guard::scan_depth() const {
		return CHAOTIC_METH_ADDICT_SCAN_DEPTH();
	}
	player_ptr_t armed_guard::spawn_near_someone() {
	...
	bool armed_guard::attack_anyone_in_same_room() {
