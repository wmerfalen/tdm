...
std::string m_config_file;
	...
	typedef std::unordered_map<std::string,std::string> conf_map;
conf_map m_settings;
conf_map m_translations;
	public: 
	conf_map& settings() { return m_settings; }
	conf_map& translations() { return m_translations; }
conf(){
}
    ...
    if(f != std::string::npos){
		...
		if(f+1 >= line.length()){
			...
			if(std::string::npos != f){
				value = line.substr(f+1,line.length() - f);
				key = line.substr(0,f - 1);
			}
		}
		util::trim(key);
		util::trim(value);
