...
#define lmdb_debug(a) /** */
	...
	_db_handle::tuple_return_type_t _db_handle::new_txn() {
		...
		if((r = mdb_txn_begin(m_env,0,0,&m_txn)) != 0) {
			...
			switch(r) {
				case MDB_PANIC:
					reason = "a fatal error occurred earlier and the environment must be shut down.";
					break;
				case MDB_MAP_RESIZED:
					reason = "another process wrote data beyond this MDB_env's mapsize and this environment's map must be resized as well. See mdb_env_set_mapsize().";
					break;
				case MDB_READERS_FULL:
					reason = "a read-only transaction was requested and the reader lock table is full. See mdb_env_set_maxreaders().";
					break;
				case ENOMEM:
	...
	_db_handle::tuple_return_type_t _db_handle::open_dbi() {
		...
		if((r = mdb_dbi_open(m_txn,0,0,&m_dbi)) != 0) {
			...
			switch(r) {
				case MDB_NOTFOUND :
					reason = "the specified database doesn't exist in the environment and MDB_CREATE was not specified.";
					break;
				case MDB_DBS_FULL :
					reason = "too many databases have been opened. See mdb_env_set_maxdbs().";
					break;
				default:
					reason += "|mdb_strerror:";
					reason += mdb_strerror(r);
 ...
 * @return
 */
pqxx::result db_get_by_meta_order_by(std::string table, std::string col,const std::string& value,std::string order_by,std::string direction) {
	try {
		auto up_txn = txn();
		sql_compositor comp(table,&up_txn);
		auto room_sql = comp.select("*")
