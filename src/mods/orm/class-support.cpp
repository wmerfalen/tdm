...
namespace mods::orm {
	...
	int16_t support::save(){
		try{
			auto up_txn = txn();
			sql_compositor comp(support::table_name,&up_txn);
			auto up_sql = comp
				.update(support::table_name)
				.set(export_class())
				.where("support_id","=",std::to_string(this->id))
				.sql();
			mods::pq::exec(up_txn,up_sql);
			mods::pq::commit(up_txn);
			return 0;
		}catch(std::exception& e){
			std::cerr << __FILE__ << ": " << __LINE__ << ": error updating support by pkid: '" << e.what() << "'\n";
			return -1;
		}
	}
	/**
	...
	uint64_t support::initialize_row(player_ptr_t &player, primary_choice_t primary_choice) {
		...
		try{
			...
			sql_compositor comp("class_support",&insert_transaction);
			...
			for(auto && row : record){
				...
				loaded = 1;
				id = support_id = row["support_id"].as<uint64_t>();
				return id;
			}
		}catch(std::exception& e){
			std::cerr << __FILE__ << ": " << __LINE__ << ": error initializing support class row: '" << e.what() << "'\n";
		}
		return 0;
	}
	...
	int16_t support::load_by_player(uint64_t player_id){
		try{
			...
			auto player_sql = comp.select("*")
				.from(table_name)
				.where("support_player_id","=",std::to_string(player_id))
				.sql();
