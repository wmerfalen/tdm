#ifndef __MENTOC_MODS_ORM_ELEVATOR_HEADER__
#define __MENTOC_MODS_ORM_ELEVATOR_HEADER__
...
namespace mods::orm {
	...
	struct elevator : public mods::orm::orm_base<elevator,std::string> {
		using primary_choice_t = std::string;
		std::string table_name() const { return "elevator"; }
		...
		int16_t save(){
			return std::get<0>(this->update<elevator>(this));
		}
