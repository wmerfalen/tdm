...
namespace mods::orm {
	...
	int16_t contagion::save(){
		try{
			auto up_txn = txn();
			sql_compositor comp(contagion::table_name,&up_txn);
			auto up_sql = comp
				.update(contagion::table_name)
				.set(export_class())
				.where("contagion_id","=",std::to_string(this->id))
				.sql();
			mods::pq::exec(up_txn,up_sql);
			mods::pq::commit(up_txn);
			return 0;
		}catch(std::exception& e){
			std::cerr << __FILE__ << ": " << __LINE__ << ": error updating contagion by pkid: '" << e.what() << "'\n";
			return -1;
		}
	}
	/**
	...
	uint64_t contagion::initialize_row(player_ptr_t &player, primary_choice_t primary_choice) {
		...
		if(primary_choice == primary_choice_t::M3){
			...
			contagion_primary_weapon_id = psg1->rifle()->attributes->flush_to_db();
			contagion_primary_type = "M3";
		}
		if(primary_choice == primary_choice_t::FAMAS){
			auto l96aw = mods::weapons::sniper_rifle::l96aw::make();
			contagion_primary_weapon_id = l96aw->rifle()->attributes->flush_to_db();
		...
		try{
			...
			sql_compositor comp("class_contagion",&insert_transaction);
			contagion_player_id = player->db_id();
			auto up_sql = comp
				.insert()
				.into("class_contagion")
				.values(export_class())
				.returning("contagion_id")
				.sql();
			...
			for(auto && row : record){
				...
				loaded = 1;
				id = contagion_id = row["contagion_id"].as<uint64_t>();
				return id;
			}
		}catch(std::exception& e){
			std::cerr << __FILE__ << ": " << __LINE__ << ": error initializing contagion class row: '" << e.what() << "'\n";
		}
		return 0;
	}
	...
	int16_t contagion::load_by_player(uint64_t player_id){
		try{
			...
			auto player_sql = comp.select("*")
				.from(table_name)
				.where("contagion_player_id","=",std::to_string(player_id))
				.sql();
