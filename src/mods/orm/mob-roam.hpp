...
namespace mods::orm {
	...
	struct mob_roam_record_t {
		...
		std::string table_name() const { return "mob_roam"; }
		std::string primary_key_name() { return "id"; }
	};

	struct mob_roam : public mods::orm::orm_base<mob_roam,std::string> {
		using primary_choice_t = std::string;
		std::string table_name() const { return "mob_roam"; }
		std::string column_prefix() const { return "mob_"; }
		std::string id_column() const { return "id"; }
		mob_roam() : id(0) {
			this->init(); loaded = 0;
		}
		~mob_roam() = default;
