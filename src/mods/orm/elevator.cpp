...
namespace mods::orm {
	uint64_t elevator::initialize_row() {
		init();
		auto status = this->create<elevator>(this);
	...
	strmap_t elevator::export_class() {
		strmap_t values;
	...
	int16_t elevator::feed(const pqxx::result::reference & row){
		init();
		loaded = 0;
