...
namespace mods::orm {
	...
	std::tuple<int16_t,std::string> player_contract_state::update_player_data(const uint64_t& in_player_id,const contract_vnum_t& in_c_vnum,const std::string& buffer) {
		this->delete_by_player_id_contract_vnum(in_player_id,in_c_vnum);
		init();
		pc_contract_vnum = in_c_vnum;
		pc_state_data = buffer;
		pc_player_id = in_player_id;
	 ...
	 */
	std::tuple<int16_t,std::string> load_player_contract_state(uint64_t in_player_id,contract_vnum_t in_c_vnum,std::string& in_buffer) {
		std::cerr << "load_player_contract_state: in_c_vnum:'" << in_c_vnum << "\n";
		using statement = std::vector<mods::orm::util::statement_t>;
		using c = std::vector<pqxx::result::reference>;
		c container;
		statement statements;
		statements.emplace_back("pc_contract_vnum","=",std::to_string(in_c_vnum),true,false);
		statements.emplace_back("pc_player_id","=",std::to_string(in_player_id),false,false);
		mods::orm::util::load_where<c,sql_compositor,statement>(
		    container,
		    player_contract_state_table_name.data(),
		    statements
		...
		for(auto c : container) {
			std::cerr << "container: '" << c["pc_state_data"] << "'\n";
		}
