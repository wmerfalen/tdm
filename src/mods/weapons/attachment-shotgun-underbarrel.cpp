...
namespace mods::weapons {
	void attachment_shotgun_underbarrel::init() {
		...
		m_variant = shotgun_variant_t::SHV_GHOST;
	}
	...
	std::tuple<bool,std::string> attachment_shotgun_underbarrel::attach_to(obj_ptr_t weapon, const uint8_t& player_tier) {
		...
		switch(m_variant) {
			default:
			case SHV_GHOST:
				set_yaml_file("sniper-class-shotgun-ub.yml");
				break;
			case SHV_MARINE:
				set_yaml_file("marine-class-shotgun-ub.yml");
				break;
		}

		m_obj = create_object(ITEM_ATTACHMENT,get_yaml_file().data());
		if(!m_obj->attachment()) {
			log(CAT("Failed to attach '",get_yaml_file().data(),"' to weapon!").c_str());
