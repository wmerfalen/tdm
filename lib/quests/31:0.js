function quest_31_0_introduction(person){
	...
	db_set(person + ":q31_deaths","0");
	db_set(person + ":quest_trigger","quest_31_0_step1('" + person + "');");
	mob_death_trigger("{character}","quest_31_0_mob_death_trigger");
}
function quest_31_0_step1(person){
	...
	if(room == 264){
		...
		db_set(person + ":quest_trigger","quest_31_0_step2('" + person + "');");
	}
}
function quest_31_0_abort(person){
	quest_abort(person,31,0);
}
function quest_31_0_step2(person){
	
}
function quest_31_0_progress(person){
	var ctr = parseInt(db_geti(person + ":q31_deaths"));
	if(ctr < 3){
		send_to_char(person,"You have eliminated {red}" + ctr + " out of 3{/red} possible HVTs");
	}
}
function quest_31_0_mob_death_trigger(person,victim,zone){
	if(victim == 'keeper peacekeeper'){
		...
		if(ctr == 3){
			...
			quest_complete(person,31,0);
		}
	}
}
quest_31_0_introduction("{character}");
